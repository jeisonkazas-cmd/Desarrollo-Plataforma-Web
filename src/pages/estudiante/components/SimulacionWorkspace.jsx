...
/**
 * Área de trabajo que contiene los paneles de simulador, informe y sidebar
 * Maneja el layout responsivo y el divisor arrastrable
 */
function SimulacionWorkspace({
  ...
  const handleDividerMouseDown = useDividerDrag(
    workspaceRef,
    onSplitChange,
    25,
    75
  ...
  const isSimulatorHidden = viewMode === 'report';
  const isReportHidden = viewMode === 'sim';
  ...
  return (
    <div className="lab-workspace" ref={workspaceRef}>
      ...
      {/* Divisor arrastrable (solo en modo split) */}
      <DraggableDivider
      ...
      {/* Sidebar de instrucciones (solo en modo sim) */}
      <InstruccionesSidebar
        practica={practica}
        isVisible={viewMode === 'sim'}
      />
    </div>
  );
}
...
SimulacionWorkspace.propTypes = {
  viewMode: PropTypes.oneOf(['sim', 'split', 'report']).isRequired,
